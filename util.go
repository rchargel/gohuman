...
// toSHA512 Creates a Base64 encoded Message Digest using SHA-512
func toSHA512(message string) string {
	c := sha512.New()
...
// newCaptchaID creates a random string for use in identifiers.
// xxxxxyyxxxxxmmxxxxxdd
func newCaptchaID(data ...string) string {
	now := time.Now()
	random := make([]byte, 32)
	rand.Read(random)
